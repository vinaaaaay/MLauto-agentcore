"""
LangChain LLM callback handler for standardized metrics emission.

Emits canonical 'llm_call' events on every LLM invocation completion.
All correlation IDs come from MetricsContext — no agent-specific ties.

This callback is TOPOLOGY-AGNOSTIC: it works identically whether the
LLM call happens inside a monolith graph or a distributed sub-agent.
"""
...
class OpenAIHeaderInterceptor(httpx.BaseTransport):
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        # ChatOpenAI uses the async path by default
...
class SessionMetricsCallback(BaseCallbackHandler):
    ...
    Captures:
        - Input/output token counts (from model usage metadata)
        - Cached token counts (if available from provider)
        - Input/output byte estimates
        - Wall-clock latency
        - All correlation IDs from MetricsContext

    Usage:
        ctx = MetricsContext()
        callback = SessionMetricsCallback(ctx=ctx, metric_logger=metric_logger)
    ...
    def on_chat_model_start(self, serialized, messages, *, run_id, parent_run_id, **kwargs):
        ...
        # Include bound tools/functions in input byte estimate
    ...
    def on_llm_end(self, response: LLMResult, *, run_id, parent_run_id, **kwargs):
        ...
        # Strategy:
        # 1. Check shared ledger (set by HeaderInterceptor) -- PREFERRED
        # 2. Check llm_output (metadata)
        # 3. Check response_metadata (message)
        
        span_id = self.ctx.span_id.get()
        openai_processing_ms = self.ctx.openai_processing_ms_ledger.get(span_id, 0.0) if span_id else 0.0
        
        if not openai_processing_ms:
            # Fallback to metadata checks if interceptor wasn't used
        ...
        # Ensure it's a number and reset the context for the next call
        try:
