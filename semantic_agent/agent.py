...
from .state import SemanticAgentState, TutorialInfo, VectorStoreMCPClient
...
def build_semantic_agent_graph(ctx=None, metric_logger=None):
    ...
    async def _mcp_tool_call(server_url: str, query: str, tool_name: str, top_k: int, condensed: bool, callbacks: list) -> List[Dict[str, Any]]:
        """
        Instrumented wrapper around the vector store MCP server retrieval.
        ...
        raw_tutorials = []
        try:
            logger.info(f"Using standard VectorStoreMCPClient for server URL: {server_url}")
