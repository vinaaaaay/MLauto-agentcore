...
def build_coder_agent_graph(ctx=None, metric_logger=None):
    ...
    Flow: generate_python_code -> generate_bash_script -> execute_and_evaluate
    """
...
"""
        ...
        if sandbox:
            ...
            logger.info(f"  Python code saved inside sandbox: {python_file_path}")
    ...
    async def generate_bash_script(state: CoderAgentState) -> dict:
        ...
        # Copy requirements files from registry on host into the sandbox
        ...
        if sandbox:
            ...
            logger.info(f"  Bash script saved inside sandbox: {bash_file_path}")
