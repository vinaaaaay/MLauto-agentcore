...
logger = logging.getLogger("mcts_handler")
...
def _get_tree(payload):
    # Retrieve mcts_tree from payload or payload itself
...
def handle_action(payload: dict) -> dict:
    ...
    if not action:
        # Fallback to init if no action is provided but perception_results/selected_tools are present
    ...
    if action == "init":
        ...
        # If perception_results is not present, check selected_tools directly in payload
        ...
        # Fallback to flat fields if current_selection is not provided
        if not selection:
            selection = {
        ...
        # Fallback to flat fields if coding_results or selection are not nested
        if not coding_results:
            coding_results = {
                ...
                "ai_call_time": payload.get("ai_call_time", 0.0)
            }
        if not selection:
            selection = {
                "node_id": payload.get("node_id"),
        ...
        if not node.get("is_successful") and coding_results.get("error_analysis"):
            ...
            # Truncate error analysis before appending to prevent blowing up limits
            analysis_str = str(coding_results.get("error_analysis"))[:1000]
            all_analyses.append(f"[Node {node_id} ({tool})] {analysis_str}")
            
        tree["all_error_analyses"] = all_analyses[-20:]  # Keep only last 20 errors
        
        # Sync tree score trackers with top-level fields for convenience
        ...
        # Fallback to flat fields if current_selection is not nested
        if not selection:
            selection = {
                "node_id": payload.get("node_id"),
                "stage": payload.get("stage"),
        ...
        # Sync top-level convenience fields
