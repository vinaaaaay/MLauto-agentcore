"""
Sandbox Client for Perception Agent.

Communicates with the MCP Server on private EC2 instances proxied through
the AWS Lambda bastion gateway. Lambda-only mode — no local execution.
...
class SandboxClient:
    """
    Sandbox Client communicating with the MCP Server on private EC2 instances
    ...
    async def write_file(self, path: str, content: str) -> bool:
        ...
        for i in range(0, len(content), chunk_size):
            ...
            inner = self._parse_inner_response(res)
            if "error" in inner or not (inner.get("success", False) or inner.get("action") == "write"):
                logger.error(f"Error writing chunk to {path}: {inner.get('error', inner)}")
                return False
        return True
